...
local function _EnsureFontStringCompat(fontString)
    if (not fontString.GetUnboundedStringWidth) and QuestieCompat and QuestieCompat.GetUnboundedStringWidth then
        fontString.GetUnboundedStringWidth = QuestieCompat.GetUnboundedStringWidth
    end
    if (not fontString.GetWrappedWidth) and fontString.GetWidth then
        fontString.GetWrappedWidth = fontString.GetWidth
    end
end
...
local function _GetTextWrapBreak(textWrapFontString, line, lineLength)
    ...
    while (endIndex <= lineLength) do
        ...
        local indexes = textWrapFontString.CalculateScreenAreaFromCharacterSpan
            and textWrapFontString:CalculateScreenAreaFromCharacterSpan(1, endIndex)
        if (not indexes) then
            local lineEndIndex, nextStartIndex, brokeAtSpace = _GetTextWrapBreakByWidth(textWrapFontString, line, lineLength)
            textWrapFontString:SetText(line)
            return lineEndIndex, nextStartIndex, brokeAtSpace
...
local function _GetRemainingRows(textWrapFontString, remainingLine, lastLine, nextStartIndex, remainingLineLength)
    local remainingIndexes = textWrapFontString.CalculateScreenAreaFromCharacterSpan
        and textWrapFontString:CalculateScreenAreaFromCharacterSpan(nextStartIndex, remainingLineLength)
...
---@param brokeAtSpace boolean Whether the candidate break is a space boundary.
---@return boolean shouldCombine True when combining improves trailing-line readability.
local function _ShouldCombineTrailingLineByRawWidth(textWrapFontString, visibleLine, rawCharactersByVisibleIndex, visibleStartIndex, nextStartIndex, remainingLineLength, brokeAtSpace)
    if (nextStartIndex > remainingLineLength) then
        return false
    end
...
local function _RebuildColorEscapedLine(visibleLine, rawCharactersByVisibleIndex, colorEventsByVisibleIndex, prefix, visibleStartIndex, visibleEndIndex)
...
---@param combineTrailing boolean? @If the last line is only one word/glyph, combine it with previous? Default: false.
...
function WrappedText:TextWrap(line, prefix, combineTrailing, desiredWidth, fontSource)
    if not textWrapObjectiveFontString then
        ...
        _EnsureFontStringCompat(textWrapObjectiveFontString)
    ...
    if (textWrapObjectiveFontString:GetUnboundedStringWidth() > textWrapObjectiveFontString:GetWrappedWidth()) then
        ...
        local function _BuildWrappedLine(visibleStartIndex, visibleEndIndex, rawLine)
            if (hasRebuildEscapes) then
                return _RebuildColorEscapedLine(measuredLine, rawCharactersByVisibleIndex, colorEventsByVisibleIndex, prefix, visibleStartIndex, visibleEndIndex)
            end
        ...
        while (startIndex <= lineLength) do
            ...
            if (hasTextureEscapes) then
                lineEndIndex, nextStartIndex, brokeAtSpace = _GetTextWrapBreakByRawWidth(textWrapObjectiveFontString, measuredLine, rawCharactersByVisibleIndex, startIndex, remainingLineLength)
            else
            ...
            if (hasTextureEscapes) then
                shouldCombineTrailingLine = combineTrailing and _ShouldCombineTrailingLineByRawWidth(textWrapObjectiveFontString, measuredLine, rawCharactersByVisibleIndex, startIndex, nextStartIndex, remainingLineLength, brokeAtSpace)
            else
                shouldCombineTrailingLine = combineTrailing and _ShouldCombineTrailingLine(textWrapObjectiveFontString, remainingLine, nextStartIndex, remainingLineLength, brokeAtSpace)
            end
