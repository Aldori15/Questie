...
local function AddHoverHighlight(button)
    ...
    elseif highlight then
        highlight:SetVertexColor(COLOR.hoverHighlight.r, COLOR.hoverHighlight.g,
            COLOR.hoverHighlight.b, COLOR.hoverHighlight.a)
    end
end
...
local function AcquireTreeRow(index)
    ...
    if treeRow.highlight.SetColorTexture then
        ...
        treeRow.highlight:SetVertexColor(COLOR.selectionHighlight.r, COLOR.selectionHighlight.g,
            COLOR.selectionHighlight.b, COLOR.selectionHighlight.a)
    end
...
function UpdateControls()
    ...
    if isActive then
        -- 3.3.5 keeps an indirection in module function slots so functions copied into locals remain measurable.
        -- A copied wrapper cannot be handed back, but Stop still removes every active measurement hook.
        if ProfilerPreHook.installed then
            SetControlTooltip(sessionButton, "Stop profiling",
                "Stops collecting and removes the measurement hooks.",
                "The 3.3.5 alias indirection remains, but does not measure.",
                "",
...
local function BuildTitleBar()
    ...
    if headerBand.SetColorTexture then
        ...
        headerBand:SetVertexColor(COLOR.chromeBand.r, COLOR.chromeBand.g, COLOR.chromeBand.b,
            COLOR.chromeBand.a)
    end
    ...
    if headerRule.SetColorTexture then
        ...
        headerRule:SetVertexColor(COLOR.divider.r, COLOR.divider.g, COLOR.divider.b, COLOR.divider.a)
    end
...
local function BuildControlRow()
    ...
    startupCheckButton:SetScript("OnClick", function(self)
        -- Stock 3.3.5 restores this saved variable just before ADDON_LOADED, so the prehook applies the change
        -- on the next reload rather than now. Saying so beats letting the checkbox look ignored.
    ...
    SetControlTooltip(startupCheckButton, "Profile on startup",
        "Starts the profiler at Questie's ADDON_LOADED event,",
        "before its normal initialization work runs.",
        "",
        "3.3.5 file-load timing is captured on every reload and",
        "is available even when profiling is started manually.",
        "",
        "Active measurement costs a little speed while it is on.",
        "Stop removes those measurement hooks.",
        "",
...
local function BuildTreePane()
    ...
    if divider.SetColorTexture then
        ...
        divider:SetVertexColor(COLOR.divider.r, COLOR.divider.g, COLOR.divider.b, COLOR.divider.a)
    end
    ...
    if treeScrollTrack.SetColorTexture then
        ...
        treeScrollTrack:SetVertexColor(COLOR.treeScrollTrack.r, COLOR.treeScrollTrack.g,
            COLOR.treeScrollTrack.b, COLOR.treeScrollTrack.a)
    end
    ...
    if treeScrollThumb.SetColorTexture then
        ...
        treeScrollThumb:SetVertexColor(COLOR.treeScrollThumb.r, COLOR.treeScrollThumb.g,
            COLOR.treeScrollThumb.b, COLOR.treeScrollThumb.a)
    end
...
local function BuildRelationPanel()
    ...
    if topRule.SetColorTexture then
        ...
        topRule:SetVertexColor(COLOR.divider.r, COLOR.divider.g, COLOR.divider.b, COLOR.divider.a)
    end
...
local function BuildFooter()
    ...
    if footerBand.SetColorTexture then
        ...
        footerBand:SetVertexColor(COLOR.chromeBand.r, COLOR.chromeBand.g, COLOR.chromeBand.b,
            COLOR.chromeBand.a)
    end
    ...
    if footerRule.SetColorTexture then
        ...
        footerRule:SetVertexColor(COLOR.divider.r, COLOR.divider.g, COLOR.divider.b, COLOR.divider.a)
    end
...
-- the command exists even on a client where profiling never armed. The 3.3.5 prehook reads the flag when
-- ADDON_LOADED restores SavedVariables, so a change only takes effect on the next reload.
