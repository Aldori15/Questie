...
-- QuestieLoader owns the clock because stock 3.3.5 has no GetTimePreciseSec. Its fallback makes
-- debugprofilestop monotonic across external debugprofilestart resets and exposes seconds to both
-- addon-load timing and runtime profiling.
---@type (fun(): number)? @Current time in milliseconds; nil when the client offers no usable timer
local Now
if type(getProfilerTime) == "function" then
    Now = function()
        return getProfilerTime() * 1000
    end
end
...
local function ShortenStackFrame(stackLine)
    ...
    -- Anchored on the addon folder rather than on "Interface", because WoW truncates a long path from the
    -- left with an ellipsis. Accept both the upstream and 3.3.5 fork folder names so anonymous jobs keep a
    -- short, stable identity on either installation.
    stackLine = string.gsub(stackLine, "[^%[%]]-[Aa]dd[Oo]ns[/\\]+[Qq]uestie%-335[/\\]+", "", 1)
    stackLine = string.gsub(stackLine, "[^%[%]]-[Aa]dd[Oo]ns[/\\]+[Qq]uestie[/\\]+", "", 1)

    -- The match above still needs the folder marker intact, and for a window of path lengths the ellipsis
    -- cut lands inside the marker itself - "...Ons/Questie-335/Modules/..." or "...335/Modules/..." - keeping
    ...
    if ellipsisFragment then
        ...
        if lowerFragment == "" or string.sub("addons", -string.len(lowerFragment)) == lowerFragment then
            local strippedPath, questieMatches = string.gsub(afterFragment, "^[Qq]uestie%-335[/\\]+", "")
            if questieMatches == 0 then
                strippedPath, questieMatches = string.gsub(afterFragment, "^[Qq]uestie[/\\]+", "")
            end
            if questieMatches > 0 then
                stackLine = strippedPath
            end
        elseif string.sub("questie-335", -string.len(lowerFragment)) == lowerFragment
            or string.sub("questie", -string.len(lowerFragment)) == lowerFragment then
...
-- The 3.3.5 compatibility layer implements the same services locally instead of registering LibStub majors.
-- Profile those owned tables as explicit roots so map/pin and dropdown work does not remain hidden inside the
-- self time of whichever Questie function called it. These are QuestieCompat fields, not arbitrary globals.
local PROFILED_COMPAT_LIBRARIES = {
    {field = "HBD", name = "Compat.HBD"},
    {
        field = "HBDPins",
        name = "Compat.HBDPins",
        frameScripts = {{field = "updateFrame", scripts = {"OnUpdate", "OnEvent"}}},
    },
    {field = "LibUIDropDownMenu", name = "Compat.LibDropDown"},
}
...
local function HookProfiledLibraries(excludedTables, visitedTables, namespaceShapeCache, questieStreamLib)
    local function HookResolvedLibrary(library, target)
    ...
    if type(LibStub) == "table" and type(LibStub.GetLibrary) == "function" then
        for _, target in ipairs(PROFILED_LIBRARIES) do
            ...
            if resolved then
                HookResolvedLibrary(library, target)
            end
        end
    end
...
local function StartProfilingSession(showUI)
    ...
    if not Now then
        Questie.Error("QuestieProfiler cannot run: this client has no supported profiling clock")
        return false, true
    end
