local questieAddonName = ... or "Questie-335"
...
-- installed at all unless startup profiling is enabled. Stock 3.3.5 is the exception: its SavedVariables are
-- unavailable while addon files execute, so this fork keeps the lightweight indirection installed and waits
-- until ADDON_LOADED to decide whether to arm measurement.
...
---Finishes load-time wrapping and starts the saved startup session exactly once.
---Called from both the early event frame and Questie:OnInitialize because 3.3.5 does not guarantee which
---ADDON_LOADED listener runs first. The OnInitialize call is the backstop that still precedes Questie's work.
function QuestieProfilerPreHook.StartIfEnabled()
    ...
    if QuestieProfilerEnabled == true then
        local profiler = QuestieLoader:ImportModule("Profiler")
        local profilerStarted, profilerArmed, rejectionReported = pcall(profiler.StartStartup, profiler, true)
...
-- 3.3.5 restores SavedVariables immediately before ADDON_LOADED. This early frame both stops the load-time
-- sweeper and, when requested on the previous session, arms profiling before Questie's normal event work.
if CreateFrame then
    ...
    finishFrame:SetScript("OnEvent", function(self, _, addonName)
        if addonName ~= questieAddonName then
            return
        end
